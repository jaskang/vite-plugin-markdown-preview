...
describe('vite', () => {
  ...
  function declareTests(isBuild) {
    ...
    test('env variables', async () => {
      ...
      expect(await getText('.custom-env-variable')).toMatch('VITE_CUSTOM_ENV_VARIABLE: 9527')
      expect(await getText('.effective-mode-file-name')).toMatch(
        `VITE_EFFECTIVE_MODE_FILE_NAME: ${isBuild ? `.env.production` : `.env.development`}`
      )
    ...
    test('config.define', async () => {
      expect(await getText('.config-define-value')).toMatch(`config.define.value: value`)
      expect(await getText('.config-define-value-from-js')).toMatch(`config.define.value from js: value`)
    })
    ...
    if (!isBuild) {
      ...
      test('hmr (vue reload)', async () => {
        await updateFile('hmr/TestHmr.vue', content => content.replace('count: ref(0),', 'count: ref(1337),'))
      ...
      test('hmr (js -> vue propagation)', async () => {
        ...
        await updateFile('hmr/testHmrPropagation.js', content => content.replace('return 1', 'return 666'))
      ...
      test('hmr (js -> vue propagation. dynamic import, static-analyzable)', async () => {
        ...
        await expectByPolling(() => getText('.hmr-propagation-dynamic'), 'bar not loaded')
      ...
      test('hmr (js -> vue propagation. full dynamic import, non-static-analyzable)', async () => {
        ...
        // update source code
        await updateFile('hmr/testHmrPropagationFullDynamicImport.js', content =>
          content.replace('baz loaded', 'baz updated')
        )
        ...
        await timeout(200)
        expect(await getText('.hmr-propagation-full-dynamic')).toBe('baz loaded')
      ...
      test('hmr (js -> vue propagation. full dynamic import, non-static-analyzable, but self-accepting)', async () => {
        ...
        expect(await getText(stateIncrementButton)).toMatch('>>> count is 1338 <<<')
        ...
        // update source code
        await updateFile('hmr/testHmrPropagationFullDynamicImportSelfAccepting.js', content =>
          content.replace('qux loaded', 'qux updated')
        )
      ...
      test('hmr (manual API, self accepting)', async () => {
        await updateFile('hmr/testHmrManual.js', content => content.replace('foo = 1', 'foo = 2'))
        await expectByPolling(
          () => browserLogs[browserLogs.length - 1],
          'js module hot updated:  /hmr/testHmrManual.js'
        )
        expect(browserLogs.slice(browserLogs.length - 4, browserLogs.length - 1)).toEqual([
          `foo was: 1`,
      ...
      test('hmr (manual API, accepting deps)', async () => {
        ...
        await updateFile('hmr/testHmrManualDep.js', content => content.replace('foo = 1', 'foo = 2'))
        await expectByPolling(
          () => browserLogs[browserLogs.length - 1],
          'js module hot updated:  /hmr/testHmrManual.js'
        )
        expect(browserLogs.slice(browserLogs.length - 8, browserLogs.length - 1)).toEqual([
    ...
    test('CSS import w/ PostCSS', async () => {
      ...
      // hmr
      if (!isBuild) {
        await updateFile('css/testPostCss.css', content => content.replace('red', 'green'))
    ...
    test('SFC <style> w/ PostCSS', async () => {
      ...
      // hmr
      if (!isBuild) {
        await updateFile('css/TestPostCss.vue', content => content.replace('color: green;', 'color: red;'))
    ...
    if (!isBuild) {
      test('hmr (style @import bail to <style>)', async () => {
        ...
        // should work
        await expectByPolling(() => getComputedColor('.postcss-from-sfc'), 'rgb(0, 128, 0)')
        await updateFile('css/TestPostCss.vue', content => {
          return content.replace(externalImport, '').replace('color: green;', 'color: red;')
        })
        // should work
        await expectByPolling(() => getComputedColor('.postcss-from-sfc'), 'rgb(255, 0, 0)')
      })

      test('hmr (style removal)', async () => {
        await updateFile('css/TestPostCss.vue', content => content.replace(/<style>(.|\s)*<\/style>/, ``))
        await expectByPolling(() => getComputedColor('.postcss-from-sfc'), 'rgb(0, 0, 0)')
      })
    }
    ...
    test('SFC <style module>', async () => {
      ...
      if (!isBuild) {
        ...
        await expectByPolling(() => getComputedColor('.css-modules-sfc'), 'rgb(0, 0, 0)')
      }
    })
    ...
    test('CSS @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(0, 128, 0)')
      if (!isBuild) {
        await updateFile('css-@import/imported.css', content => content.replace('green', 'rgb(0, 0, 0)'))
        await expectByPolling(() => getComputedColor(el), 'rgb(0, 0, 0)')
      }
    })
    ...
    test('SFC <style> w/ @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(255, 0, 0)')
      if (!isBuild) {
        await updateFile('css-@import/testCssAtImportFromStyle.css', content => content.replace('red', 'rgb(0, 0, 0)'))
        await expectByPolling(() => getComputedColor(el), 'rgb(0, 0, 0)')
      }
    })
    ...
    test('CSS module @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(0, 128, 0)')
      if (!isBuild) {
        await updateFile('css-@import/imported.module.css', content => content.replace('green', 'rgb(0, 0, 0)'))
        await expectByPolling(() => getComputedColor('.sfc-script-css-module-at-import'), 'rgb(0, 0, 0)')
      }
    })
    ...
    test('SFC <style module> w/ @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(255, 0, 0)')
      if (!isBuild) {
        await updateFile('css-@import/testCssModuleAtImportFromStyle.module.css', content =>
    ...
    test('CSS preprocessor @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(0, 128, 0)')
      if (!isBuild) {
        await updateFile('css-@import/testScss.imported.scss', content => content.replace('green', 'rgb(0, 0, 0)'))
        await expectByPolling(() => getComputedColor(el), 'rgb(0, 0, 0)')
      }
    })
    ...
    test('SFC <style lang="sass"> w/ @import', async () => {
      ...
      expect(await getComputedColor(el)).toBe('rgb(255, 0, 0)')
      if (!isBuild) {
        await updateFile('css-@import/testScssAtImportFromStyle.scss', content =>
          content.replace('red', 'rgb(0, 0, 0)')
        )
        await expectByPolling(() => getComputedColor(el), 'rgb(0, 0, 0)')
      }
    ...
    test('import *.module.css', async () => {
      ...
      if (!isBuild) {
        ...
        // css module results in component reload so must use fresh selector
        await expectByPolling(() => getComputedColor('.css-modules-import'), 'rgb(0, 0, 1)')
      }
    })
    ...
    test('import *.module.scss', async () => {
      ...
      if (!isBuild) {
        ...
        // css module results in component reload so must use fresh selector
        await expectByPolling(() => getComputedColor('.scss-modules-import'), 'rgb(0, 0, 2)')
      }
    })
    ...
    test('pre-processors', async () => {
      ...
      expect(await (await getEl('.pug')).evaluate(el => el.getAttribute('id'))).toMatch('')
      expect(await getComputedColor('.pug')).toBe('rgb(255, 0, 255)')
      if (!isBuild) {
        await updateFile('TestPreprocessors.vue', c => c.replace('$color: magenta', '$color: black'))
        await expectByPolling(() => getComputedColor('.pug'), 'rgb(0, 0, 0)')
        await updateFile('TestPreprocessors.vue', c => c.replace('rendered from', 'pug with hmr'))
    ...
    test('SFC src imports', async () => {
      ...
      if (!isBuild) {
        ...
        // template
        await updateFile('src-import/template.html', c => c.replace('{{ msg }}', '{{ msg }} changed'))
    ...
    test('json', async () => {
      ...
      if (!isBuild) {
        await updateFile('json/testJsonImport.json', c => c.replace('this is json', 'with hmr'))
    ...
    test('typescript', async () => {
      ...
      if (!isBuild) {
        await updateFile('ts/TestTs.vue', c => c.replace(`m: string = 'from ts'`, `m: number = 123`))
        await expectByPolling(() => getText('.ts-self'), '123')

        await updateFile('ts/testTs.ts', c => c.replace(`n: number = 1`, `n: number = 2`))
    ...
    test('alias', async () => {
      ...
      expect(await getText('.dir-alias-index')).toMatch('directory alias index works')
      expect(await getText('.dir-alias-import-outside')).toMatch('directory aliased internal import outside works')
      if (!isBuild) {
        await updateFile('alias/aliased/index.js', c => c.replace('works', 'hmr works'))
        await expectByPolling(() => getText('.alias'), 'alias hmr works')

        await updateFile('alias/aliased-dir/named.js', c => c.replace('works', 'hmr works'))
        await expectByPolling(() => getText('.dir-alias'), 'directory alias hmr works')

        await updateFile('alias/aliased-dir/index.js', c => c.replace('works', 'hmr works'))
        await expectByPolling(() => getText('.dir-alias-index'), 'directory alias index hmr works')

        await updateFile('alias/aliased-dir-import.js', c => c.replace('works', 'hmr works'))
        await expectByPolling(
    ...
    test('transforms', async () => {
      ...
      if (!isBuild) {
        await updateFile('transform/testTransform.scss', c => c.replace('cyan', 'rgb(0, 0, 0)'))
        await expectByPolling(() => getComputedColor(el), 'rgb(0, 0, 0)')
        await updateFile('transform/testTransform.js', c => c.replace('= 1', '= 2'))
    ...
    test('monorepo support', async () => {
      ...
      if (!isBuild) {
        ...
        await updateFile(`optimize-linked/index.js`, c => c.replace(`foo()`, `123`))
    ...
    test('SFC custom blocks', async () => {
      ...
      if (!isBuild) {
        await updateFile('custom-blocks/TestCustomBlocks.vue', c => c.replace('hello,vite!', 'hi,vite!'))
        await expectByPolling(() => getText('.custom-block'), 'hi,vite!')
        await updateFile('custom-blocks/TestCustomBlocks.vue', c => c.replace(`useI18n('en')`, `useI18n('ja')`))
        await expectByPolling(() => getText('.custom-block'), 'こんにちは')
      }
    })

    test('normalize publicPath', async () => {
      expect(await getText('.normalize-public-path')).toMatch(JSON.stringify([2, 4]))
    })
    ...
    test('<script setup> and <style vars>', async () => {
      ...
      await expectByPolling(() => getComputedColor(`.style-vars`), 'rgb(0, 128, 0)')
      if (!isBuild) {
        // test <script setup HMR>
        await updateFile('script-setup/TestScriptSetupStyleVars.vue', c => c.replace(`ref('red')`, `ref('blue')`))
        await expectByPolling(() => getComputedColor(`.style-vars`), 'rgb(0, 0, 255)')
        // #748
        await updateFile('script-setup/TestScriptSetupStyleVars.vue', c => c.replace(`msg="hello"`, `msg="hi"`))
        await expectByPolling(() => getText('.test-script-setup-child'), 'Child has prop hi')
      }
    })
  ...
  describe('build', () => {
    ...
    test('css codesplit in async chunks', async () => {
      ...
      const css = await fs.readFile(path.join(tempDir, 'dist/_assets', cssFile), 'utf-8')
      // should be extracted from the main css file
      expect(css).not.toMatch(colorToMatch)
      // should be inside the split chunk file
      const asyncChunk = files.find(f => f.startsWith('TestAsync') && f.endsWith('.js'))
      const code = await fs.readFile(path.join(tempDir, 'dist/_assets', asyncChunk), 'utf-8')
  ...
  describe('dev', () => {
    ...
    test('hmr (index.html full-reload)', async () => {
      ...
      await updateFile('index.html', content => content.replace('Vite App', 'Vite App Test'))
      await reload
    ...
    test('hmr (html full-reload)', async () => {
      ...
      await updateFile('test.html', content => content.replace('Vite App', 'Vite App Test'))
      await reload
      await expectByPolling(() => getText('title'), 'Vite App Test')
    })
    ...
    test('page reload cache busting', async () => {
      ...
      await updateFile('json/testJsonImport.json', c => c.replace('with hmr', 'with page reload'))
